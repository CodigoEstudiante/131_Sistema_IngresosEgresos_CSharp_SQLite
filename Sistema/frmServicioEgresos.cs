...
namespace Sistema
{
    public partial class frmServicioEgresos : Form
    {
        ...
        {
            using (var form = new mdServicioIngreso(true, null, "EGRESOS"))
            {
        ...
        private void mostrar_servicios()
        {
            ...
            string mensaje;
            List<Servicio> olista = ServicioLogica.Instancia.Listar(out mensaje).Where(s => s.TipoServicio == "EGRESOS").ToList();
            ...
            else if (dgdata.Columns[e.ColumnIndex].Name == "btnEditar")
            {
                if (index >= 0)
                {
                    Servicio oservicio = new Servicio()
                    {
                        ...
                        TipoServicio = "EGRESOS"
                    };

                    using (var form = new mdServicioIngreso(false, oservicio, "EGRESOS"))
                    {
                        var result = form.ShowDialog();
                        if (result == DialogResult.OK)
                        {
